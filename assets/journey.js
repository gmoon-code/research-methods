  ...
  function methodStatus(p){
    ...
    try{
      if(window.RMSMethods){
        const r=window.RMSMethods.readiness(p);
        out={available:true,score:r.score,label:r.label,critical:r.critical,warning:r.warning,ethicsStatus:r.ethicsStatus,
             locked:!!(p.methods?.protocolVersions||[]).length};
      }
    }catch{}
    return out;
  }

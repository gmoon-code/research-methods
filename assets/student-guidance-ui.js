...
window.RMSGuidanceUI=(()=>{
 ...
 function labPanel(lab,tab){
   ...
   return `<div class="lab-guide"><div><strong>${E(h.title)}</strong><p>${E(h.text)}</p>${(h.decisionKeys||[]).map(k=>choiceHelp(k)).join("")}<button class="ghost small lab-rescue-button" data-rescue-lab="${E(lab)}|${E(tab)}">I’m stuck · open progressive help</button></div><details><summary>Terms used here</summary>${termsHTML(h.terms||[])}</details></div>`;
 }
 function glossaryModal(initial=""){
   ...
   wrap.innerHTML=`<div class="modal glossary-modal"><div class="journey-head"><div><h3>Research Terms & Clarifications</h3><p>${E(G.novicePromise||"You are not expected to know these words before using the site.")}</p></div><button class="ghost small" id="closeGlossary">Close</button></div><label><span>Search a research term</span><input id="glossarySearch" value="${E(initial)}" placeholder="Try binary, paired, confounder, p-value…"></label><div id="glossaryList" class="glossary-list">${renderList(initial)}</div></div>`;
 ...
 return {stagePanel,fieldHelp,choiceHelp,labPanel,termsHTML,glossaryModal,guideModal,bindDelegated,termDef};
})();

...
(() => {
  ...
  function renderNav(){
    $("phaseNav").innerHTML=C.phases.map(ph=>`<div class="phase"><div class="phase-title">${esc(ph.label)}</div>${ph.steps.map(id=>{
      const s=stage(id);return `<button class="nav-step ${project.currentStage===id?"active":""} ${project.ready[id]?"done":""}" data-stage="${id}"><span class="nav-num">${project.ready[id]?"✓":id}</span><span class="nav-label">${esc(s.nav)}</span></button>`
    }).join("")}</div>`).join("");
    ...
    if(mobile){mobile.innerHTML=C.stages.map(s=>`<option value="${s.id}" ${project.currentStage===s.id?"selected":""}>${s.id}. ${esc(s.nav)}</option>`).join("");mobile.onchange=()=>{project.currentStage=Number(mobile.value);save();renderAll();window.scrollTo({top:210,behavior:"smooth"})}}
  }
  ...
  function fieldHTML(f){
    const [key,label,type,options]=f, val=project.data[key]??"";
    if(type==="textarea") return `<label><span>${esc(label)}</span><textarea data-field="${key}">${esc(val)}</textarea></label>`;
    if(type==="select") return `<label><span>${esc(label)}</span><select data-field="${key}"><option value="">Choose…</option>${options.map(o=>`<option ${val===o?"selected":""}>${esc(o)}</option>`).join("")}</select></label>`;
    return `<label><span>${esc(label)}</span><input data-field="${key}" value="${esc(val)}"></label>`;
  }

  function sectionsHTML(s){
    return s.sections.map(sec=>`<div class="form-section"><h4>${esc(sec.title)}</h4><p>${esc(sec.desc)}</p><div class="form-grid ${sec.fields.length>4?"two":""}">${sec.fields.map(fieldHTML).join("")}</div></div>`).join("");
  }
  ...
  function interestCompass(){
    return `<div class="tool-card"><h4>Interest Compass</h4><p>Use this after writing your own interests. It generates question pathways, not a finished topic.</p>
  ...
  function searchBuilder(){
    return `<div class="tool-card"><h4>Boolean Search Builder</h4><p>Enter synonyms separated by commas. The builder creates a draft query that you should adapt to the database syntax.</p>
  ...
  function designMatcher(){
    return `<div class="tool-card"><h4>Research Design Matcher</h4><p>Answer based on what your question is trying to know. The result is a recommendation to evaluate, not an automatic method assignment.</p>
  ...
  function dataSchemaBuilder(){
    ...
    return `<div class="tool-card"><h4>Data Table & Dictionary Builder</h4><p>Define columns before collecting final data. The CSV template will contain headers only. Never use blank cells to mean several different things.</p>
  ...
  function renderStage(){
    ...
    $("stageView").innerHTML=`<article class="card stage-card">
      <header class="stage-header"><div class="stage-meta"><span class="phase-pill">${esc(C.phases.find(p=>p.id===s.phase).label)}</span><span class="stage-count">Stage ${s.id} of ${C.stages.length}</span></div><h2>${esc(s.title)}</h2><div class="purpose">${esc(s.purpose)}</div></header>
      ...
      <div class="tab-panel" id="tabLearn"><div class="lesson-grid"><div class="lesson">${s.learn}</div><aside>${`<div class="example-box">${s.example}</div><div class="warning-box">${s.warning}</div>`}</aside></div></div>
      <div class="tab-panel hidden" id="tabWork"><div class="work"><h3>Your research notebook</h3>${coachPanel()}${customHTML(s)}${s.id>=15&&s.id<=17?`<div class="tool-card"><h4>Scientific Writing Laboratory</h4><p>Use the evidence maps and section-specific audits while drafting this part of the paper.</p><button class="secondary" id="openWritingFromStage">Open Writing Lab</button></div>`:""}${sectionsHTML(s)}</div></div>
      <div class="tab-panel hidden" id="tabCheck"><div class="check"><h3>Readiness check</h3><p>Mark a stage ready only when these statements are true. You can return later and revise.</p><div class="readiness">${s.checks.map((c,i)=>`<label class="ready-item"><input type="checkbox" data-ready-check="${i}"><span>${esc(c)}</span></label>`).join("")}</div>${s.id===4?'<button class="secondary" id="checkRQ">Run question wording check</button><div id="rqFeedback"></div>':""}<div class="ready-controls"><span class="ready-status ${project.ready[s.id]?"good":""}">${project.ready[s.id]?"Stage marked ready":"Not yet marked ready"}</span><button class="${project.ready[s.id]?"secondary":"primary"}" id="markReady">${project.ready[s.id]?"Mark as not ready":"Mark stage ready"}</button></div></div></div>
    </article>`;
    bindStage();
  }
  ...
  function mInput(id,label,val="",type="text",options=[]){
    if(type==="textarea")return `<label><span>${esc(label)}</span><textarea id="${id}">${esc(val)}</textarea></label>`;
    if(type==="select")return `<label><span>${esc(label)}</span><select id="${id}"><option value="">Choose…</option>${options.map(o=>`<option ${val===o?"selected":""}>${esc(o)}</option>`).join("")}</select></label>`;
    return `<label><span>${esc(label)}</span><input id="${id}" value="${esc(val)}"></label>`;
  }
  ...
  function methodsDesignTab(){
    const d=project.methods.design||{}, p=Methods.designProfile(project), a=Methods.auditDesign(project);
  ...
  function methodsReplicationTab(){
    const d=project.methods.design||{}, r=Methods.auditReplication(project);
  ...
  function methodsSamplingTab(){
    const s=project.methods.sampling||{}, a=Methods.auditSampling(project);
  ...
  function methodsMeasurementTab(){
    const a=Methods.auditMeasurements(project);
  ...
  function methodsEthicsTab(){
    const e=project.methods.ethics||{}, r=Methods.ethicsRoute(project);
  ...
  function methodsProcedureTab(){
    const a=Methods.procedureAudit(project);
  ...
  function methodsDataTab(){
    const d=project.methods.design||{}, a=Methods.dataSchemaAudit(project);
  ...
  function methodsAuditTab(){
    const r=Methods.readiness(project), versions=project.methods.protocolVersions||[];
  ...
  function bindMethodsWorkspace(rerender,active){
    ...
    if($("lockProtocol")) $("lockProtocol").onclick=()=>{
      const r=Methods.readiness(project);
      if(r.critical||r.ethicsStatus==="do_not_facilitate"){alert("Resolve critical issues before locking the protocol.");return}
      project.methods.protocolVersions.push(Methods.protocolSnapshot(project));save();rerender("audit");
    };
  ...
  function bindStage(){
    ...
    if($("saveIndependent")) $("saveIndependent").onclick=()=>{
      const r=Coach.reviewStage(current().id,project);
    ...
    if($("reviewAI")) $("reviewAI").onclick=async()=>{
      ...
      const local=Coach.reviewStage(current().id,project);
      const payload={
        stage_id:current().id,
        ...
        course_config:{audience:"secondary student",citation_style:project.data.citationStyle||"APA 7"}
      };
      try{
    ...
    if($("markReady")) $("markReady").onclick=()=>{project.ready[current().id]=!project.ready[current().id];save();renderAll()};

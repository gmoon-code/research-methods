...
(() => {
  ...
  function fieldHTML(f){
    ...
    let control="";
    if(type==="textarea") control=`<label><span>${esc(label)}</span><textarea data-field="${key}">${esc(val)}</textarea></label>`;
    else if(type==="select") control=`<label><span>${esc(label)}</span><select data-field="${key}"><option value="">Choose…</option>${options.map(o=>`<option ${val===o?"selected":""}>${esc(o)}</option>`).join("")}</select></label>`;
    else control=`<label><span>${esc(label)}</span><input data-field="${key}" value="${esc(val)}"></label>`;
    return `<div class="guided-field">${control}${Guide.fieldHelp(key,label)}</div>`;
  }
  ...
  function renderStage(){
    ...
    $("stageView").innerHTML=`<article class="card stage-card">
      ...
      <div class="tab-panel hidden" id="tabWork"><div class="work"><h3>Your research notebook</h3>${Guide.stagePanel(s.id,true)}${coachPanel()}${customHTML(s)}${s.id>=15&&s.id<=17?`<div class="tool-card"><h4>Scientific Writing Laboratory</h4><p>Use the evidence maps and section-specific audits while drafting this part of the paper.</p><button class="secondary" id="openWritingFromStage">Open Writing Lab</button></div>`:""}${sectionsHTML(s)}</div></div>
  ...
  function mInput(id,label,val="",type="text",options=[]){
    ...
    return `<div class="guided-field">${control}${hit?`<button type="button" class="term-inline-help" data-open-glossary="${hit}">What does ${esc(hit)} mean?</button>`:""}</div>`;
  }

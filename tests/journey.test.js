...
window.RMSCoach={reviewStage:()=>({score:85,label:"Strong",messages:[]})};
...
let p={
  created:"2026-01-01",
  name:"Test",
  ready:{1:true,2:true,3:true,4:true,5:true,6:true,7:true,8:true,9:true,10:true,11:true,12:true},
  data:{
    finalRQ:"Does X relate to Y?",
    questionType:"Correlational / observational",
    designType:"Correlational / observational",
    gapStatement:"Local extension",
    primaryEstimand:"correlation"
  },
  sources:[
  ...
  methods:{protocolVersions:[{}]},
  analysis:{rawData:[],runs:[]},
  writing:{sections:{discussion:"",conclusion:"",abstract:""}},
  journey:{}
};
...
if(ms[0].state!=="complete")process.exit(1);
if(ms[1].state!=="complete")process.exit(1);
if(ms[2].state!=="complete")process.exit(1);
if(J.currentFocus(p).kind!=="work")process.exit(1);

J.requestCheckpoint(p,"M1");
if(J.milestoneStatus(p,J.milestoneDefs[0]).state!=="complete")process.exit(1);
...
let fb=J.makeTeacherFeedbackPacket(packet,{
  checkpoints:[{id:"M1",status:"approved",teacher:"T",comment:"Approved",conditions:[]}],
  feedback:[],
  competency_ratings:[{competency:"question_formulation",level:2}]
});
...
console.log("PASS independent journey model with legacy packet compatibility");

# Research Methods Studio v2.16.0 FREE architecture

## Release objective

v2.16.0 retains the zero-cost Cloudflare Workers AI Research Chat architecture and adds authenticated teacher access. The static application remains suitable for GitHub Pages. Research Chat uses Cloudflare Workers AI through a Worker binding and requires no OpenAI API key or other third-party model credential.
...
5. Keep the v2.16.0 model lock at `@cf/meta/llama-3.3-70b-instruct-fp8-fast` unless a later release revalidates another model.
...
There is no OpenAI API call in the v2.16.0 executable Research Chat path. The Worker calls `env.AI.run()` through the Cloudflare Workers AI binding.
...
## Teacher authentication architecture

Teacher access uses the same Cloudflare Worker and a separate credential
boundary. `RMS_TEACHER_ACCESS_CODE` authenticates teacher login.
`RMS_TEACHER_SESSION_SECRET` signs teacher-session tokens. Student Research
Chat continues to use `RMS_CHAT_ACCESS_CODE`.

The public browser contains none of those values. `teacher.html` sends a
teacher credential directly to the Worker login route. After successful
authentication, the browser keeps only the signed session token in
`sessionStorage`. The Worker verifies that token before teacher tools are
exposed.

The checked-in production `workers.dev` URL is public configuration. It is not
a secret.

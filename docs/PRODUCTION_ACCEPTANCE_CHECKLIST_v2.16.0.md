# Production acceptance checklist — v2.16.0 FREE
...
- [ ] `RMS_TEACHER_ACCESS_CODE` is stored as a separate Worker secret and is 16?256 characters.
- [ ] `RMS_TEACHER_SESSION_SECRET` is stored as a cryptographically random Worker secret.
...
- [ ] authenticated GET health reports `version=2.16.0`.
- [ ] `/teacher/session` accepts the configured teacher code from the approved GitHub Pages origin.
- [ ] `/teacher/session/verify` accepts a newly issued signed teacher token.
- [ ] a tampered teacher token is rejected.
- [ ] an unapproved origin is rejected for teacher authentication.
- [ ] the student Research Chat class code still authenticates successfully after teacher-auth deployment.

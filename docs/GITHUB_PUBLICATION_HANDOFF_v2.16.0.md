# GitHub publication handoff — v2.16.0 FREE

**Status**  Do not publish yet. The v2.16 release manifest and complete release gate must pass before this handoff procedure is used.


## Purpose

This document covers the **first GitHub publication step only**. It publishes the exact frozen v2.16.0 FREE repository to a review branch. It does not merge `main`, enable GitHub Pages, deploy Cloudflare, or activate Research Chat.
...
- Extract the final v2.16.0 publication-handoff ZIP to its own folder.
- Install Git 2.x.
...
6. replace the temporary worktree contents with the frozen v2.16.0 release;
...
`HEAD:refs/heads/release/v2.16.0-free-public`
...
If `release/v2.16.0-free-public` already exists, the helper refuses to overwrite it.

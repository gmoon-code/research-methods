...
def main():
    result={"release":"v2.13","checks":[],"page_errors":[],"console_errors":[],"screenshots":[],"measurements":{}}
    ...
    with sync_playwright() as pw:
      ...
      ck("Welcome has one obvious student action",page.locator("button:visible").count()==1,page.locator("button:visible").all_inner_texts())
      ...
      ck("Floating AI does not compete with Start before a project exists",page.locator("#aiHelperLauncher:visible").count()==0)
      ...
      ck("Example opens in one contextual Help surface",all(x in htxt for x in ["Too vague","Good working response","Help me get unstuck","Ask Research AI"]))
      ...
      # 8. Student More menu and AI fallback.
      page,errs,cons=new_page(browser,seed_stage4());result["page_errors"]+=errs;result["console_errors"]+=cons
      ...
      ck("Disconnected AI uses student-friendly fallback language","AI help is unavailable right now" in atxt and "server-side" not in atxt.lower())
      page.close()
      ...
      ck("Mobile AI launcher keeps an understandable label","Ask AI" in page.locator("#aiHelperLauncher").inner_text())
    ...
    (OUT/"BROWSER_QA_RESULTS_v2.13.json").write_text(json.dumps(result,indent=2,ensure_ascii=False),encoding="utf-8")

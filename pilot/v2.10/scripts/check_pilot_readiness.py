...
freeze=subprocess.run([sys.executable,str(ROOT/'pilot/v2.10/scripts/verify_pilot_freeze.py')],capture_output=True,text=True)
print(freeze.stdout.strip())
if freeze.returncode:
    print('PILOT READINESS: NO-GO — frozen baseline changed')
    sys.exit(2)
with open(ROOT/'pilot/v2.10/forms/PRE_FLIGHT_CHECKLIST.csv',newline='',encoding='utf-8') as f: checks=list(csv.DictReader(f))
with open(ROOT/'pilot/v2.10/forms/PILOT_ISSUE_LOG.csv',newline='',encoding='utf-8') as f: issues=list(csv.DictReader(f))

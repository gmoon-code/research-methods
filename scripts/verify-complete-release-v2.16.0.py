...
BROWSER_GATES = [
    ...
    ['node','tests/preview-immutability-v216.mjs'],
...
def static_release_checks()->None:
    required=[
        'index.html','404.html','teacher.html','assets/curriculum.js','assets/app.js','assets/style.css','assets/runtime-config.js',
        'assets/ai-adapter.js','assets/ai-helper.js','assets/ai-helper-ui.js','assets/teacher-session.js','assets/word-export.js','api/research-chat.js',
        'api/package.json','package.json','backend/cloudflare-workers-ai/worker.mjs','backend/cloudflare-workers-ai/teacher-auth.mjs','backend/cloudflare-workers-ai/wrangler.jsonc',
        ...
        'docs/FREE_ZERO_COST_ARCHITECTURE_v2.16.0.md','docs/CLOUDFLARE_FREE_DEPLOYMENT_v2.16.0.md',
        'docs/PRODUCTION_ACCEPTANCE_CHECKLIST_v2.16.0.md','docs/FREE_RELEASE_AUDIT_v2.16.0.md',
        'docs/OFFLINE_RELEASE_VERIFICATION_v2.16.0.md','docs/PUBLIC_GITHUB_READINESS_v2.16.0.md','docs/PUBLIC_GITHUB_PUBLICATION_AUDIT_v2.16.0.md','docs/GITHUB_PUBLICATION_HANDOFF_v2.16.0.md','SECURITY.md',
        '.gitattributes','scripts/verify-public-github-readiness-v2.16.0.py','scripts/prepare-github-publication-v2.16.0.py','tests/github-publication-helper-v216.py'
    ]
    missing=[p for p in required if not (ROOT/p).is_file()]
    if missing: raise RuntimeError('Missing required v2.16.0 files: '+', '.join(missing))
    ...
    runtime=(ROOT/'assets/runtime-config.js').read_text(encoding='utf-8').strip()
    prefix='window.RMS_RUNTIME_CONFIG = Object.freeze('
    suffix=');'

    if not runtime.startswith(prefix) or not runtime.endswith(suffix):
        raise RuntimeError(
            'runtime-config.js does not use the expected '
            'RMS_RUNTIME_CONFIG Object.freeze wrapper.'
        )

    try:
        runtime_config=json.loads(
            runtime[len(prefix):-len(suffix)]
        )
    except json.JSONDecodeError as exc:
        raise RuntimeError(
            f'runtime-config.js payload is not valid JSON: {exc}'
        ) from exc

    if runtime_config.get('version')!='2.16.0':
        raise RuntimeError(
            'runtime-config.js is not the v2.16.0 production config.'
        )

    if runtime_config.get('freeEdition') is not True:
        raise RuntimeError(
            'runtime-config.js lost the FREE-edition flag.'
        )

    for key in ('researchChatEndpoint','chatEndpoint'):
        if runtime_config.get(key)!=PRODUCTION_CHAT_ENDPOINT:
            raise RuntimeError(
                f'{key} is not locked to the validated '
    ...
    expected_secrets={'RMS_CHAT_ACCESS_CODE','RMS_TEACHER_ACCESS_CODE','RMS_TEACHER_SESSION_SECRET'}
    if required_secrets!=expected_secrets:
        raise RuntimeError('Worker secret declaration does not match the validated Chat + teacher-auth contract.')
    ...
    current_docs='\n'.join((ROOT/p).read_text(encoding='utf-8') for p in [
        Path('README.md'),Path('START_HERE.md'),Path('docs/FREE_ZERO_COST_ARCHITECTURE_v2.16.0.md'),Path('docs/CLOUDFLARE_FREE_DEPLOYMENT_v2.16.0.md')
    ])
...
def artifact_and_secret_scan()->None:
    ...
    for p in ROOT.rglob('*'):
        ...
        # The verifier runs from a normal Git working clone.
        # Ignore only this checkout's own top-level .git metadata.
        # Any nested .git directory remains a forbidden artifact.
...
def manifest_check()->None:
    path=ROOT/'release-manifest-v2.16.json'
    if not path.is_file(): raise RuntimeError('release-manifest-v2.16.json is missing.')
...
def main()->int:
    ap=argparse.ArgumentParser(description='Verify Research Methods Studio v2.16.0 FREE complete release.')
    ap.add_argument('--skip-manifest',action='store_true')
    args=ap.parse_args()
    print('Research Methods Studio v2.16.0 FREE complete-release verification')
    ...
    if args.skip_manifest:
        print(
            'SKIP frozen v2.16 manifest and publication-source check '
            '(--skip-manifest)'
        )
    else:
        manifest_check()
    ...
    run(
        'Public GitHub readiness gate',
        [
            sys.executable,
            'scripts/verify-public-github-readiness-v2.16.0.py'
        ]
    )

    if not args.skip_manifest:
        run(
            'GitHub publication helper frozen-source check',
            [
                sys.executable,
                'scripts/prepare-github-publication-v2.16.0.py',
                '--check'
            ]
        )

        run(
            'GitHub publication helper offline branch-safety QA',
            [
                sys.executable,
                'tests/github-publication-helper-v216.py'
            ]
        )
    else:
        print(
            'SKIP publication-helper frozen-source and branch-safety QA '
            '(both require the frozen v2.16 manifest)'
        )
    ...
    print('\nCOMPLETE v2.16.0 FREE RELEASE VERIFICATION: PASS')
    print('Production Chat and teacher authentication are deployed and smoke-tested; final v2.16 production acceptance still requires the versioned production revalidation step.')
    return 0

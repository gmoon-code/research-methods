#!/usr/bin/env python3
"""Safely publish the frozen v2.16.0 public release to a GitHub release-candidate branch.
...
VERSION = "2.16.0"
MANIFEST_NAME = "release-manifest-v2.16.json"
...
def run_release_verification(skip_full: bool) -> None:
    if skip_full:
        ...
        return
    run([sys.executable, "scripts/verify-complete-release-v2.16.0.py"], cwd=ROOT, capture=False)


def publication_check(*, run_full: bool = False, skip_full: bool = False) -> str:
    require_git()
    verify_release_manifest(ROOT)
    run([sys.executable, "scripts/verify-public-github-readiness-v2.16.0.py"], cwd=ROOT, capture=False)
    if run_full:
    ...
    with tempfile.TemporaryDirectory(prefix="rms-v215-publish-") as td:
        ...
        try:
            ...
            git(["-c", "commit.gpgsign=false", "commit", "--no-verify", "-m",
                 "Release v2.16.0 free public candidate"], cwd=worktree, capture=False)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Prepare/publish the exact Research Methods Studio v2.16.0 public release to a release-only Git branch."
    )
